...
assert(js.includes('if (isCompleteDetailAd(instantAd)) {') && js.includes('renderDynamicAdDetail(instantAd);') && js.includes('} else {\n          beginDynamicDetailPending();'), 'Incomplete one-thumbnail cache can still render immediately');
...
assert(js.includes('complete cache before creating any') || js.includes('temporary 1/1 gallery'), 'Regression rationale marker missing');
