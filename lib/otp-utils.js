...
function parseRequestBodyValue(value, maxBytes, tooLargeMessage, invalidMessage) {
  if (value === undefined || value === null || value === '') return {};
  if (Buffer.isBuffer(value)) value = value.toString('utf8');
  if (typeof value === 'string') {
    if (Buffer.byteLength(value, 'utf8') > maxBytes) throw new Error(tooLargeMessage);
    try { return value ? JSON.parse(value) : {}; }
    catch (_) { throw new Error(invalidMessage); }
  }
...
function readBody(req) {
  ...
  if (req.body !== undefined && req.body !== null) {
    return Promise.resolve().then(() => parseRequestBodyValue(
      req.body,
      maxBytes,
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('end', () => {
      if (settled) return;
      try { resolve(parseRequestBodyValue(body, maxBytes, 'Request body too large', 'Invalid JSON body')); }
