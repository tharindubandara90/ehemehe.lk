...
function imageProxy(id, index = 0, version = '') {
  const base = `/api/ad-image?id=${encodeURIComponent(String(id))}&index=${index}`;
...
function normalizeAd(row, detail = false) {
  ...
  const imageVersion = row.updated_at || row.created_at || custom.updated_at || custom.submitted_at || '';
  const publicImages = id
    ? Array.from({ length: proxyCount }, (_, index) => imageProxy(id, index, imageVersion))
    : [];
  return {
    id,
