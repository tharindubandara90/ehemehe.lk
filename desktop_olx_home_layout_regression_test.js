...
assert(css.includes('grid-template-columns: minmax(320px, 1.5fr)'), 'Search/category/location layout is missing');
...
assert(js.includes("categorySelect.innerHTML = '<option value=\"\">All categories</option>'"), 'Category selector is missing');
assert(js.includes("latestTitle.textContent = 'Fresh recommendations'"), 'Fresh recommendations heading is missing');
assert(js.includes("params.set('category', category)"), 'Category search parameter is not submitted');
